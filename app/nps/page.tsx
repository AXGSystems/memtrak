...
export default function NPS() {
  return (
    <div className="p-6">
      ...
      <SampleDataBadge message="MEMTRAK does not yet ingest survey responses, so the score, response counts, segment breakdowns and feedback themes below are illustrative sample values — not computed from real member surveys." />

      {/* NPS Score Hero */}
      <div className="rounded-xl border p-6 mb-6 text-center" style={{ background: 'var(--card)', borderColor: 'var(--card-border)' }}>
        <div className="text-4xl sm:text-6xl font-extrabold" style={{ color: npsData.score >= 50 ? C.green : npsData.score >= 30 ? C.blue : C.orange }}>+{npsData.score}</div>
        ...
        <div className="flex justify-center gap-6 mt-4">
          <div><div className="text-lg font-extrabold" style={{ color: C.green }}>{npsData.promoters}%</div><div className="text-[11px]" style={{ color: 'var(--text-muted)' }}>Promoters (9-10)</div></div>
          <div><div className="text-lg font-extrabold" style={{ color: 'var(--text-muted)' }}>{npsData.passives}%</div><div className="text-[11px]" style={{ color: 'var(--text-muted)' }}>Passives (7-8)</div></div>
          <div><div className="text-lg font-extrabold" style={{ color: C.red }}>{npsData.detractors}%</div><div className="text-[11px]" style={{ color: 'var(--text-muted)' }}>Detractors (0-6)</div></div>
        </div>
      </div>
        ...
        <div className="space-y-2">
          {npsData.topReasons.map(r => (
            <div key={r.reason} className="flex items-center gap-3 p-3 rounded-lg" style={{ background: 'var(--background)' }}>
              <span className={`text-[11px] px-2 py-0.5 rounded-full font-bold ${r.sentiment === 'positive' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>{r.sentiment === 'positive' ? '👍' : '👎'}</span>
