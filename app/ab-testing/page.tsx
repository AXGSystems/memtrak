...
export default function ABTesting() {
  return (
    <div className="p-6">
      <h1 className="text-lg font-extrabold text-white mb-6">A/B Testing</h1>

      <div className="space-y-6">
        {tests.map(test => (
          <div key={test.id} className="bg-[var(--card)] border border-[var(--card-border)] rounded-xl p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="text-right">
                ...
                <div className="text-[10px] text-white/40">lift for {test.winner === 'A' ? 'Variant A' : 'Variant B'}</div>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-4 mb-4">
              <div className={`p-4 rounded-lg ${test.winner === 'A' ? 'bg-green-500/10 border-2 border-green-500/30' : 'bg-white/5 border border-white/10'}`}>
                ...
                <div className="text-[10px] text-white/50 mb-2">{test.a.subject}</div>
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div><div className="text-sm font-extrabold text-white">{test.a.openRate}%</div><div className="text-[9px] text-white/40">Open Rate</div></div>
                  <div><div className="text-sm font-extrabold text-white">{test.a.opened}</div><div className="text-[9px] text-white/40">Opened</div></div>
                  <div><div className="text-sm font-extrabold text-white">{test.a.clicked}</div><div className="text-[9px] text-white/40">Clicked</div></div>
                </div>
              </div>
              <div className={`p-4 rounded-lg ${test.winner === 'B' ? 'bg-green-500/10 border-2 border-green-500/30' : 'bg-white/5 border border-white/10'}`}>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-bold text-white">Variant B</span>
                  {test.winner === 'B' && <span className="text-[9px] px-2 py-0.5 rounded-full bg-green-500/20 text-green-400 font-bold">WINNER</span>}
                </div>
                <div className="text-[10px] text-white/50 mb-2">{test.b.subject}</div>
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div><div className="text-sm font-extrabold text-white">{test.b.openRate}%</div><div className="text-[9px] text-white/40">Open Rate</div></div>
                  <div><div className="text-sm font-extrabold text-white">{test.b.opened}</div><div className="text-[9px] text-white/40">Opened</div></div>
                  <div><div className="text-sm font-extrabold text-white">{test.b.clicked}</div><div className="text-[9px] text-white/40">Clicked</div></div>
                </div>
              </div>
            </div>
